% Use the Euclidean distance of the FT of two signals to extract a sine from noise
clear;
Fs=16000;
f=1000;
N=2048;
...
s=zeros(length(Xmi_),1);
for k_=1:length(Xmi_)
    l_=(Xmi_(k_)-1);
    Wc(:,k_)=db2p(Xm_(k_))*mper(exp(j*2*pi*l_/N_dft*n),w,N_dft,N);
    mask=((n_dft >= (l_/N_dft*N-B)) & (n_dft <= (l_/N_dft*N+B)));
    s(k_)=norm((Wc(:,k_)-X).*mask,2);
end
figure(1);
